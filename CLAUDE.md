...
- Weibo's API returns `data['ok']` as an int, not a bool: `-100` means the cookie is logged out, `1` means healthy. Cookie refresh requires a Playwright-driven login flow (see `scripts/drive_my_chrome.py` for the "control the user's own visible Chrome via CDP" pattern used for anything requiring a real login).
...
- 微博的 API 把 `data['ok']` 返回成 int 而不是 bool：`-100` 表示 Cookie 已登出，`1` 表示健康。刷新 Cookie 需要一套 Playwright 驱动的登录流程（见 `scripts/drive_my_chrome.py`，那是「通过 CDP 控制用户自己那个看得见的 Chrome」的模式，任何需要真实登录的场景都用它）。
