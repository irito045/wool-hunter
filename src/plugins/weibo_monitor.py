...
# bot 连不上 NapCat 时，微博轮询只能空转。以前每轮只打一条一模一样的 WARNING，
# 于是「bot 活着、端口在听、控制台显示运行中，但一条消息都收发不了」这个状态
# 对用户是**完全无声**的——他看到的只有「今天怎么没羊毛」，和 2026-07-19 那次
# AI 判定全挂的症状一模一样，两种完全不同的故障给出同一个表象，最难查。
# 2026-07-18 21:00 真实发生过：连续空转 12 分钟，日志里只有 7 条重复 WARNING。
# 超过阈值就升级成 ERROR 并报出「已经持续多久」，控制台「运行」页一眼能看见。
_not_ready_streak: int = 0
_NOT_READY_ALERT_AFTER = 3      # 连续几轮未就绪后升级为 ERROR
_NOT_READY_REPEAT_EVERY = 10    # 之后每隔几轮再提醒一次，别刷屏
...
async def _check_weibo():
    ...
    global _fail_streak, _alert_active, _fresh_start, _not_ready_streak
    ...
    if bot is None:
        _not_ready_streak += 1
        # 刚启动/NapCat 重连的头几轮是正常的（NapCat 有 30s 重连间隔），先按 WARNING 记；
        # 连续超过阈值说明它根本没连上，升级 ERROR 并报出已持续时长。
        if _not_ready_streak < _NOT_READY_ALERT_AFTER:
            logger.warning("[微博] Bot 实例未就绪，跳过本轮检查")
        elif (_not_ready_streak == _NOT_READY_ALERT_AFTER
              or _not_ready_streak % _NOT_READY_REPEAT_EVERY == 0):
            mins = _not_ready_streak * WEIBO_CHECK_INTERVAL // 60
            logger.error(
                f"[微博] bot 已持续约 {mins} 分钟没连上 NapCat，微博源一直在空转、"
                f"一条羊毛也收不到（已跳过 {_not_ready_streak} 轮）。"
                f"多半是 NapCat 没登录或掉线了，去控制台「运行」页看看。"
            )
        return
    if _not_ready_streak:
        logger.info(f"[微博] bot 已连上，恢复检查（此前空转 {_not_ready_streak} 轮）")
        _not_ready_streak = 0
